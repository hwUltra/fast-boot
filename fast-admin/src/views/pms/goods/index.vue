...
import { getGoodsList } from "@/api/pms/goods";
import { getOptions } from "@/api/pms/category";
...
const shopOptions = ref<OptionType[]>();
...
onMounted(async () => {
  ...
  await getShopOptions().then((response) => {
    shopOptions.value = response.data.list;
    if (shopOptions.value != undefined) {
      shopId = parseInt(shopOptions.value[0].value);
    }
  });
  await getOptions(shopId).then(({ data }) => {
  ...
  <div class="app-container">
    <div class="search">
      <el-form ref="queryFormRef" :model="queryParams" :inline="true">
        ...
        <el-form-item label="商户" prop="parentId">
          <el-tree-select
