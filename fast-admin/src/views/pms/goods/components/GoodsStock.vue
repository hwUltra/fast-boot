...
function generateSkuList() {
  ...
  console.log("state.skuForm.skuList ", state.skuForm.skuList);
}
  ...
  if (currSpecValue) {
    if (specValues && specValues.length > 0) {
      ...
      let maxSpecValueIndex = specValues
        .filter((item: any) => item.idStr.includes("tid_"))
        .map((item: any) => item.idStr.split("_")[2])
        .reduce((acc: any, curr: any) => {
          return acc > curr ? acc : curr;
        }, 0);
      state.specForm.specList[rowIndex].values[specValues.length] = {
        value: currSpecValue,
        idStr: "tid_" + (rowIndex + 1) + "_" + ++maxSpecValueIndex,
      };
    } else {
      state.specForm.specList[rowIndex].values = [
        { value: currSpecValue, idStr: "tid_" + (rowIndex + 1) + "_1" },
      ];
    }
  }
