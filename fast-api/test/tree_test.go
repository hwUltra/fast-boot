...
func TestGenerateTree(t *testing.T) {
	...
	//resp = tree.GenerateTree(SystemMenus.ConvertToINodeArray(allMenu), SystemMenus.ConvertToINodeArray(selectedNode))
	...
	//// 查询 设备 的所有父节点
	//respNodes := tree.FindRelationNode(SystemMenus.ConvertToINodeArray(device), SystemMenus.ConvertToINodeArray(allMenu))
	//resp = tree.GenerateTree(respNodes, nil)
	//bytes, _ = json.Marshal(resp)
	//fmt.Println(string(pretty.Color(pretty.PrettyOptions(bytes, pretty.DefaultOptions), nil)))
}
