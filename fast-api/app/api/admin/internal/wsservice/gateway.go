...
func (g *Gateway) sendToAll(msgBean *core.MsgBean) {
	if g.Ws.WsClient.Role != "admin" {
		msg := core.ReturnBeanFail("你没有权限发送群消息")
		_ = g.Ws.WsClient.SendMessage(websocket.TextMessage, msg)
		return
	}
	g.Ws.SendToAll(core.ReturnBeanSuccess(ReturnDataBean{
		Type:      "sendAll",
		FromType:  "user",
...
func (g *Gateway) sendToUid(msgBean *core.MsgBean) {
	...
	g.Ws.SendToUid(uid, core.ReturnBeanSuccess(ReturnDataBean{
		Type:      "sendUid",
		FromType:  "user",
		FromUid:   g.Ws.WsClient.Uid,
		FromGroup: 0,
...
func (g *Gateway) sendToGroup(msgBean *core.MsgBean) {
	g.Ws.SendToGroup(msgBean.To, core.ReturnBeanSuccess(ReturnDataBean{
		Type:      "sendGroup",
		FromType:  "Group",
		Msg:       msgBean.Data,
		FromUid:   g.Ws.WsClient.Uid,
