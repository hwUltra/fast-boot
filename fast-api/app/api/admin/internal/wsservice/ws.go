...
func OnOpen(hub *wsCore.Hub, rw http.ResponseWriter, r *http.Request, c config.Config) (*Ws, bool) {
	if client, ok := (&wsCore.Client{}).OnOpen(hub, rw, r); ok {
		...
		//uid := jwtx.GetUidByToken(c.Auth.AccessSecret, token)
		//上线用上面
		uid, _ := strconv.ParseInt(token, 10, 64)
		logc.Infof(r.Context(), "【ws-open】 %s uid: %d ", token, uid)
...
func (w *Ws) JoinGroup(uid int64, groupId string) error {
	...
	if ok, _ := w.RedisClient.Sismember("Group", groupId); ok == false {
		return xerr.NewErrCodeMsg(10001, "群组不存在")
	}
	if _, err := w.RedisClient.Sadd(groupId, uid); err != nil {
		return xerr.NewErrCodeMsg(10001, "加入失败")
	}
	return nil
}
...
func (w *Ws) LeaveGroup(uid int64, groupId string) error {
	...
	if _, err := w.RedisClient.Srem(groupId, uid); err != nil {
		return xerr.NewErrCodeMsg(10001, "退出失败")
	}
	return nil
}
