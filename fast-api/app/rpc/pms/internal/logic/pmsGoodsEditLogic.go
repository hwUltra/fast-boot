...
func (l *PmsGoodsEditLogic) PmsGoodsEdit(in *pmsPb.PmsGoodsForm) (*pmsPb.SuccessIdResp, error) {
	...
	if err := l.svcCtx.GormConn.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(model.PmsGoodsAttributeModel{}).Where("1=1").Update("`status`", 0).Error; err != nil {
			return err
		}
		...
		if err := tx.Model(model.PmsSkuModel{}).Where("1=1").Update("`status`", 0).Error; err != nil {
			return err
		}
